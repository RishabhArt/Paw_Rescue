...
const AuthPage = () => {
  ...
  // Setup auth state listener FIRST, then fetch existing session
  useEffect(() => {
    ...
    try {
      ...
      if (err.message?.includes('provider') || err.message?.includes('not enabled')) {
        toast({ 
          ...
          description: "Google sign-in is not yet configured. Please use email/password or contact the administrator.", 
          variant: "destructive" 
        });
      } else {
        toast({ 
  ...
  const handleGithubLogin = async () => {
    ...
    try {
      ...
      if (err.message?.includes('provider') || err.message?.includes('not enabled')) {
        toast({ 
          title: "OAuth Not Configured", 
          description: "GitHub sign-in is not yet configured. Please use email/password or contact the administrator.", 
          variant: "destructive" 
        });
      } else {
        toast({ 
      ...
      <Card className="w-full max-w-md bg-card border-border">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardTitle className="text-2xl">
            {isLogin ? "Welcome Back" : "Join PawRescue"}
          </CardTitle>
          <CardDescription>
            {isLogin ? "Sign in to continue helping animals" : "Create your account to make a difference"}
          </CardDescription>
        </CardHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Button type="button" variant="ghost" size="sm" className="absolute right-2 top-1.5 h-7 w-7 p-0" onClick={() => setShowPassword(!showPassword)}>
